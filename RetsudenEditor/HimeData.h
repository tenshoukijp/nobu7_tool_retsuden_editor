...
char* szHimeTargetFileName = "message.nb7";
...
public:
	...
	// message.nb7→配列へと格納。
	int HimeData_Import() {
		if (!System::IO::File::Exists(gcnew String(szHimeTargetFileName))) {
			System::Windows::Forms::MessageBox::Show(gcnew String(szHimeTargetFileName) + "を読み込めません", "エラー", MessageBoxButtons::OK, ::MessageBoxIcon::Error);
		}

		// 圧縮されたLS11フォーマットをデコードする関数
		ls11_DecodePack(szHimeTargetFileName, NULL, &vBufHimeDecodedData);
	...
	std::string GetOneHimeExportData(int iSelectedIndex) {
	...
	BOOL GetOneHimeImportData(String^ managedFilePath, int iSelectedIndex) {
	...
	BOOL AllSaveToBFileN6P() {
		...
		// メモリ→パック化イメージ
		int result = ls11_EncodePack(szHimeTargetFileName, NULL, &vBufHimeDecodedData);

		if (result == 0) {
			System::Windows::Forms::MessageBox::Show(gcnew String(szHimeTargetFileName) + "へと出力しました。", "完了", MessageBoxButtons::OK, ::MessageBoxIcon::Information);
			return TRUE;
		}
		else {
