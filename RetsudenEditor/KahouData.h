...
public:
	...
	int KahouData_Import() {
		...
		ls11_DecodePack(szKahouTargetFileName, NULL, &vBufKahouDecodedData );
	...
	BOOL AllSaveToMessageN6P() {
		...
		// メモリ→パック化イメージ
		int result = ls11_EncodePack( szKahouTargetFileName, NULL, &vBufKahouDecodedData );
	...
	void MakeSplittedDataToJoindData(int iStartIndex, int iAssingIndex) {
		...
		// それを全体データに加える。
		vBufKahouDecodedData[iAssingIndex] = vDstJoinedData;
	}
